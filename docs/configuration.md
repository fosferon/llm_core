...
This document explains how HuMan/DevMan deployments configure `llm_core`, how
the layered TOML file is merged, and how to inspect or edit runtime settings
using the new mix tasks.
...
2. **Global override** – `~/.devman/llm_core.toml`
...
via `HINDSIGHT_*` env vars. CommBus uses the same defaults, so HuMan and DevMan
can point at a shared TOML without further tweaks.
...
custom TOML file before copying it into HuMan/DevMan repos.

## Integrating with HuMan & DevMan

1. Drop the generated `config/llm_core.toml` into each project (or symlink to a
   shared copy inside your workspace).
2. Add per-project overrides (e.g. HuMan-specific routing) via
   `mix llm_core.config.set --file apps/human/config/llm_core.toml ...`.
3. Ensure CI sets the necessary env vars (`ANTHROPIC_API_KEY`, etc.) – the new
   auto-discovery will also look for `LLM_CORE_<ALIAS>_API_KEY` to simplify
   local development.
4. Telemetry defaults log pipeline spans to the console; adjust `telemetry`
   section or run `mix llm_core.config.set --path telemetry.enable_logger --value false --type boolean`
   for quiet environments.

With these tools HuMan and DevMan can share a single orchestration library,
while still layering provider aliases, cost tiers, and memory defaults per
environment.
