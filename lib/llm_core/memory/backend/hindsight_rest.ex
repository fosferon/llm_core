defmodule LlmCore.Memory.Backend.HindsightREST do
  @moduledoc """
  ...
  def do_retain(url, content, metadata, opts) do
    ...
    api_key = Keyword.get(opts, :api_key)
    body = Request.retain_payload(content, metadata, false, opts)
  ...
  def rest_post(base_url, path, body, timeout, opts \\ []) do
    ...
    headers = build_headers(api_key, resolved: Keyword.get(opts, :api_key_resolved, false))
  ...
  @doc false
  def build_headers(api_key \\ nil, opts \\ []) do
    headers = [{"content-type", "application/json"}]

    key =
      if Keyword.get(opts, :resolved, false) do
        api_key
      else
        case api_key do
  ...
  @doc false
  @spec report_result({:ok, term()} | {:error, term()}, term() | nil) :: :ok
  def report_result(result, namespace \\ nil)
  def report_result({:ok, _}, nil), do: CircuitBreaker.report_success()
  def report_result({:ok, _}, namespace), do: CircuitBreaker.report_success(namespace)
  def report_result({:error, reason}, nil), do: CircuitBreaker.report_failure(reason)

  def report_result({:error, reason}, namespace),
    do: CircuitBreaker.report_failure(reason, namespace)

  @doc false
  @spec resolve_bank_id(keyword()) :: String.t()
  def resolve_bank_id(opts) do
    Request.resolve_bank_id(opts, Config.effective_bank_id())
  end

  @doc false
  @spec normalize_budget(atom() | String.t() | nil) :: String.t()
  defdelegate normalize_budget(budget), to: Request
